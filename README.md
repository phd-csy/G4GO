...
- `--backend auto|geant4|optix`：选择传输后端，默认 `auto`。
...
- `--threads UINT32`：Geant4 backend 的 worker 数量；auto 和 OptiX 使用串行 Geant4 事件驱动。
...
| `geant4` | 由 Geant4 跟踪 | Geant4 | Geant4 SD |
| `optix` | 捕获后终止 Geant4 光子 | OptiX 9.1 | `SensorHC` / `SensorHit` |
...
OptiX 的编译验证覆盖 device IR、IR embedding 和 host pipeline 链接。`auto` backend 需要驱动提供完整的 `libnvoptix.so.1` API；部分 WSL 环境只提供 loader shim，缺少 `optixQueryFunctionTable`，此时运行阶段会自动切换到 Geant4。显式使用 `--backend optix` 时会报告驱动能力不足。
