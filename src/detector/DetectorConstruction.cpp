...
auto DetectorConstruction::Construct() -> G4VPhysicalVolume* {
    ...
    const std::vector<double> scintillatorEnergyBin{
        3.300891 * eV, 3.394291 * eV, 3.459551 * eV, 3.515883 * eV, 3.557591 * eV, 3.591915 * eV, 3.622042 * eV,
        3.644458 * eV, 3.678815 * eV, 3.690132 * eV, 3.715531 * eV, 3.728362 * eV, 3.747776 * eV, 3.768708 * eV,
        3.787216 * eV, 3.80725 * eV,  3.820723 * eV, 3.847958 * eV, 3.871416 * eV, 3.9022 * eV,   3.932045 * eV,
        4.042466 * eV, 4.157667 * eV, 4.193192 * eV, 4.224366 * eV, 4.24096 * eV,  4.257685 * eV, 4.274542 * eV,
        4.291534 * eV, 4.306942 * eV, 4.343328 * eV, 4.359111 * eV, 4.396388 * eV, 4.437953 * eV, 4.471035 * eV,
        4.511202 * eV, 4.546346 * eV, 4.602552 * eV, 4.662178 * eV, 4.733725 * eV, 4.883613 * eV, 5.114986 * eV};
    const std::vector<double> scintillationComponent1{
        0.152652, 0.190301, 0.233538, 0.282997, 0.328661, 0.379536, 0.430119, 0.47672,  0.531663, 0.57447,  0.617499,
        0.656175, 0.695466, 0.729048, 0.768509, 0.803861, 0.841795, 0.88913,  0.937399, 0.977291, 1.0,      0.972946,
        0.880537, 0.821459, 0.754517, 0.691979, 0.640116, 0.591668, 0.540922, 0.501151, 0.452269, 0.413595, 0.35813,
        0.307095, 0.264469, 0.218419, 0.182053, 0.129528, 0.081253, 0.047185, 0.017148, 0.0};
    const std::vector<double> sensorEnergyBin{1.771068 * eV, 2.101763 * eV, 2.478823 * eV, 2.66882 * eV,
                                              2.756137 * eV, 2.849362 * eV, 2.949113 * eV, 3.02425 * eV,
                                              3.099632 * eV, 3.178868 * eV, 3.3981 * eV,   3.649811 * eV,
                                              3.877416 * eV, 4.128759 * eV, 4.348776 * eV};
    const std::vector<double> sensorEfficiency{0.26319,  0.395706, 0.503681, 0.525767, 0.555215,
                                               0.570552, 0.597546, 0.59816,  0.599387, 0.593252,
                                               0.521472, 0.466258, 0.458896, 0.408589, 0.196933};
    ...
    const auto physicalWorld{
        new G4PVPlacement(G4Transform3D{}, logicalWorld, "World", nullptr, false, 0, fCheckOverlap)};
    ...
    const auto solidCoupler{new G4Box("Coupler", crystalWidth / 2, crystalWidth / 2, coupleThickness / 2)};
    const auto logicalCoupler{new G4LogicalVolume(solidCoupler, siliconeGrease, "Coupler")};
    new G4PVPlacement(transform(coupleThickness / 2), logicalCoupler, "Coupler", logicalWorld, false, 0, fCheckOverlap);

    const auto solidWindow{new G4Box("Window", sipmWidth / 2, sipmWidth / 2, windowThickness / 2)};
    ...
    const auto physicalCrystal{
        new G4PVPlacement(G4Transform3D{}, logicalCrystal, "Crystal", logicalWorld, false, 0, fCheckOverlap)};
    ...
    const auto windowPositionZ{crystalLength / 2 + coupleThickness + windowThickness / 2};
    ...
    for (int i{}; i < sipmArraySize; ++i) {
        for (int j{}; j < sipmArraySize; ++j) {
            ...
            new G4PVPlacement(
                ...
                logicalWindow, "Window", logicalWorld, false, i * sipmArraySize + j, fCheckOverlap);
            new G4PVPlacement(
                G4Translate3D{
                    G4ThreeVector{sipmPositionX, sipmPositionY, sipmPositionZ}
            },
                logicalSiPM, "SiPM", logicalWorld, false, i * sipmArraySize + j, fCheckOverlap);
        }
    }
...
auto DetectorConstruction::ConstructSDandField() -> void {
    auto scintillatorSD{new ScintillatorSD("ScintillatorSD", "ScintillatorHitsCollection")};
    G4SDManager::GetSDMpointer()->AddNewDetector(scintillatorSD);
    SetSensitiveDetector("Crystal", scintillatorSD, true);

    auto sensorSD{new SensorSD(SensorSensitiveDetectorName, "SensorHitsCollection")};
