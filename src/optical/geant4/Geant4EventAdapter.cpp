...
auto Geant4EventAdapter::BeginEvent(G4int eventID) -> void {
    ...
    fEmissions.clear();
    fEventStatistics = {};
    fEventStatistics.fValidFields =
        StatisticFieldBit(PhotonTransportStatisticField::Generated);
...
auto Geant4EventAdapter::CaptureOffloaded(const G4Track& track) -> void {
    const auto appendTrackContext = [this, &track](
        ...
        description << "event " << fEventID << ", track "
                    << track.GetTrackID() << ", material ";
        if (material == nullptr) {
            description << "<null>";
        } else {
    ...
    if (const auto* info{G4GOQuasiScintillationTrackInfo::Cast(
...
auto Geant4EventAdapter::CaptureScintillation(
    ...
    if (properties == nullptr) {
        ...
        description << "scintillation offload metadata references material "
                    << data.mat_index << " without optical properties for "
                    << "event " << fEventID << ", track "
                    << track.GetTrackID();
        if (material != nullptr) {
    ...
    if (componentIndex < 0 || componentIndex > 2) {
        G4ExceptionDescription description{};
        description << "invalid scintillation component index "
                    << componentIndex << " for event " << fEventID
    ...
    if (components.at(static_cast<std::size_t>(componentIndex)) == nullptr) {
        G4ExceptionDescription description{};
        description << "scintillation component " << componentIndex
                    << " is missing for event " << fEventID << ", track "
                    << track.GetTrackID() << ", material ";
        if (material == nullptr) {
            description << "<null>";
        } else {
    ...
    emission.fDeltaVelocityMmPerNs =
        static_cast<float>(data.delta_velocity / (mm / ns));
    emission.fDecayTimeNs = static_cast<float>(info.ScintTime() / ns);
    emission.fRiseTimeNs = static_cast<float>(info.RiseTime() / ns);
    emission.fCharge = static_cast<float>(data.charge);
    emission.fSpectrumID = static_cast<std::uint32_t>(componentIndex);
