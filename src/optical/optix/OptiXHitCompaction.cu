...
auto QueryOptiXHitCompactionBytes(std::size_t count) -> std::size_t {
    ...
    const auto error{cub::DeviceSelect::Flagged(
        nullptr, bytes, static_cast<const DevicePhotonHit*>(nullptr), static_cast<const std::uint32_t*>(nullptr),
        static_cast<DevicePhotonHit*>(nullptr), static_cast<std::uint32_t*>(nullptr), count, nullptr)};
...
auto CompactOptiXHits(CUdeviceptr inputHits, CUdeviceptr inputFlags, CUdeviceptr outputHits, CUdeviceptr outputCount,
    ...
    return cub::DeviceSelect::Flagged(
        ...
        reinterpret_cast<const DevicePhotonHit*>(inputHits), reinterpret_cast<const std::uint32_t*>(inputFlags),
        reinterpret_cast<DevicePhotonHit*>(outputHits), reinterpret_cast<std::uint32_t*>(outputCount), count, stream);
}
