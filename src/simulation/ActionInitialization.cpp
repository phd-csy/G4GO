...
auto ActionInitialization::Build() const -> void {
    ...
    const auto adapter{std::make_shared<G4GO::Optical::Geant4EventAdapter>(fConfiguration, fBatchScheduler)};

    SetUserAction(new PrimaryGeneratorAction{fConfiguration.seed});
